...
public class CopyOfTouchesOnTheLineActivity extends BaseCsoundActivity implements
	...
	private static final float TOUCH_MOVE_THRESHOULD = 0.1f;
	...
	public static final String webserver = "http://wscompmus.deusanyjunior.dj/cscores";
//	public static final String webserver = "http://10.0.2.1:3000/cscores";
...
//				Log.i(TAG, score);
		...
		if (started) {
			Toast.makeText(this, "Connected to webserver", 1000).show();
		} else {
	...
	public synchronized static void setScoreReaderRunning(boolean isScoreReaderRunning) {
		CopyOfTouchesOnTheLineActivity.scoreReaderRunning = isScoreReaderRunning;
	}
	...
	private void playCSoundScore(int userId, String csoundScore) {
		...
		for(String lineScore: splittedScore) {
			...
			// Action Move	
			if(csoundScore.startsWith("m")) {
				String temp[] = lineScore.split(" ");
				long delay = Long.parseLong(temp[1].substring(1));
				SystemClock.sleep(delay);
